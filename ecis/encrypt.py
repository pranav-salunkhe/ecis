...
# Updated hyperchaotic system parameters
a, b, c, d, e = 0.1, 0.1, 14, 0.1, 0.42
x0, y0, z0, u0, v0 = 0, 0, 0, 0, 0

# Updated hyperchaotic system definition
def hyperchaotic(X, t, a, b, c, d, e):
    x, y, z, u, v = X
    x_dot = -y - z
    y_dot = x + a*y
    z_dot = b + z*(x - c)
    u_dot = d*(u - v)
    v_dot = e*u + v*(x - u - z)
    return x_dot, y_dot, z_dot, u_dot, v_dot
...
def securekey (iname):
    ...
    update_hyperchaotic(key.hexdigest())

    return key.hexdigest() ,m ,n


#########
# Function to update hyperchaotic system with the key
def update_hyperchaotic(key):
    key_bin = bin(int(key, 16))[2:].zfill(256)
    k = np.array(list(map(int, key_bin)))

    global x0, y0, z0, u0, v0
    x0 += np.sum(k[:50]) / 50
    y0 += np.sum(k[50:100]) / 50
    z0 += np.sum(k[100:150]) / 50
    u0 += np.sum(k[150:200]) / 50
    v0 += np.sum(k[200:]) / 56
#########
...
def gen_chaos_seq(m,n):
    ...
    f = odeint(hyperchaotic, (x0, y0, z0), t, args=(a, b, c))
    x, y, z = f.T
    x=x[:(N)]
    y=y[:(N)]
    z=z[:(N)]
...
def gen_hyperchaos_seq(m, n):
    global x0, y0, z0, u0, v0, a, b, c, d, e, N
    N = m * n * 4
    x = np.zeros(N)
    y = np.zeros(N)
    z = np.zeros(N)
    ...
    v = np.zeros(N)
    t = np.linspace(0, tmax, N)
    f = odeint(hyperchaotic, (x0, y0, z0, u0, v0), t, args=(a, b, c, d, e))
    x, y, z, u, v = f.T
    x = x[:N]
    y = y[:N]
    z = z[:N]
